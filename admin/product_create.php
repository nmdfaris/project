        ...
        if(!empty($productName) || !empty($productPrice) || !empty($productCategory) || !empty($productDescription)){
            ...
            if(empty($submitErr)){
                if(move_uploaded_file($_FILES['file']['tmp_name'],$real_target_dir)){
					...
					$sql_insertData = "INSERT INTO product (name, category, price, description, picture) VALUES ('$productName', '$productCategory', '$productPrice', '$productDescription', '$new_image_name')";
